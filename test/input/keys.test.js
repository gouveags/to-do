...
describe('parseKey', () => {
    ...
    it('parses Escape key', () => {
        ...
        assert.deepStrictEqual(key, { name: 'escape', ctrl: false, sequence: '\x1b' });
    });
    it('parses Ctrl+C', () => {
    ...
    it('parses up arrow', () => {
        ...
        assert.deepStrictEqual(key, { name: 'up', ctrl: false, sequence: '\x1b[A' });
    });
    it('parses down arrow', () => {
        const key = parseKey(Buffer.from('\x1b[B'));
        assert.deepStrictEqual(key, { name: 'down', ctrl: false, sequence: '\x1b[B' });
    });
    it('parses right arrow', () => {
        const key = parseKey(Buffer.from('\x1b[C'));
        assert.deepStrictEqual(key, { name: 'right', ctrl: false, sequence: '\x1b[C' });
    });
    it('parses left arrow', () => {
        const key = parseKey(Buffer.from('\x1b[D'));
        assert.deepStrictEqual(key, { name: 'left', ctrl: false, sequence: '\x1b[D' });
    });
    it('parses backspace', () => {
        const key = parseKey(Buffer.from('\x7f'));
        assert.deepStrictEqual(key, { name: 'backspace', ctrl: false, sequence: '\x7f' });
    });
    it('parses tab', () => {
