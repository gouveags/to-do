...
import { saveTodo, loadTodo, listTodos, deleteTodo, ensureStorageDir, STORAGE_DIR } from '../../src/storage/todos.ts'
...
describe('storage', () => {
  ...
  describe('saveTodo', () => {
    ...
    it('saves todo with correct content', () => {
      ...
      saveTodo(todo, TEST_DIR)
      const content = fs.readFileSync(path.join(TEST_DIR, 'test-2.json'), 'utf8')
  ...
  describe('listTodos', () => {
    ...
    it('returns all saved todos', () => {
      ensureStorageDir(TEST_DIR)
      const todo1: Todo = { id: 't1', title: 'First', items: [], createdAt: 1000, updatedAt: 2000 }
      const todo2: Todo = { id: 't2', title: 'Second', items: [], createdAt: 2000, updatedAt: 3000 }
    ...
    it('returns todos sorted by updatedAt descending', () => {
      ensureStorageDir(TEST_DIR)
      const todo1: Todo = { id: 't1', title: 'Older', items: [], createdAt: 1000, updatedAt: 1000 }
      const todo2: Todo = { id: 't2', title: 'Newer', items: [], createdAt: 2000, updatedAt: 3000 }
      saveTodo(todo1, TEST_DIR)
      saveTodo(todo2, TEST_DIR)
      const todos = listTodos(TEST_DIR)
  ...
  describe('deleteTodo', () => {
    it('deletes todo file', () => {
      ensureStorageDir(TEST_DIR)
      const todo: Todo = { id: 'del-1', title: 'To Delete', items: [], createdAt: 1000, updatedAt: 2000 }
      saveTodo(todo, TEST_DIR)
